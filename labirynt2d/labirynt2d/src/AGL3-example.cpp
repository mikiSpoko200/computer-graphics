...
namespace _2d {
    ...
    struct Cross : public components::IDrawable<>, IFixedMovable, IFixedRotatable {
        ...
        const GLuint ROTATION_UNIFORM = 3;
        const GLuint COLOR_UNIFORM = 2;

        components::StateManager<> gl_manager;

        struct {
            KinematicSegment vertical;
            KinematicSegment horizontal;
        } kinematic_ctx;
        ...
        Cross(Segment vertical, Segment horizontal, glm::vec2 velocity, GLfloat angular_velocity) {
            this->draw_ctx.vertical = vertical;
            this->draw_ctx.horizontal = horizontal;

            this->kinematic_ctx.vertical.base = vertical;
            this->kinematic_ctx.horizontal.base = horizontal;

            this->kinematic_ctx.vertical.set_velocity(velocity);
            this->kinematic_ctx.vertical.set_angular_velocity(angular_velocity);
            this->kinematic_ctx.horizontal.set_velocity(velocity);
            this->kinematic_ctx.horizontal.set_angular_velocity(angular_velocity);
    ...
    protected:
        void register_shaders() override {
            this->gl_manager.compile_shaders_from_file("shaders/cross_v.glsl", "shaders/cross_f.glsl"); ASSERTGL();
        }

        void register_buffers() override {
            DEBUGLN("S1:");
            DEBUGFMTLN("  P1: %f, %f", draw_ctx.vertical.p1.x, draw_ctx.vertical.p1.y);
            DEBUGFMTLN("  P2: %f, %f", draw_ctx.vertical.p2.x, draw_ctx.vertical.p2.y);
            DEBUGLN("S2:");
            DEBUGFMTLN("  P1: %f, %f", draw_ctx.horizontal.p1.x, draw_ctx.horizontal.p1.y);
            DEBUGFMTLN("  P2: %f, %f", draw_ctx.horizontal.p2.x, draw_ctx.horizontal.p2.y);

            this->gl_manager.bind_buffer<>();

            glBufferData(GL_ARRAY_BUFFER, sizeof(Cross::draw_ctx), this->draw_ctx.segments, GL_STATIC_DRAW); ASSERTGL();
        ...
        void draw() override {
            ...
            const auto center = this->kinematic_ctx.horizontal.base.center();
            const auto angle = this->kinematic_ctx.horizontal.base.angle();

            glUniform1f(Cross::SCALE_UNIFORM, 5.0f / 16.0f);
            ASSERTGL();
            glUniform2f(Cross::TRANSLATION_UNIFORM, center.x, center.y);
            ASSERTGL();
            glUniform3f(Cross::COLOR_UNIFORM, cross_color.r, cross_color.g, cross_color.b);
            ASSERTGL();
            DEBUGFMTLN("%f", angle);
            const glm::mat2 rotation = {
                { std::cos(angle), -std::sin(angle) },
            ...
            DEBUGFMTLN("rotation: %f, %f, %f, %f", rotation[0][0], rotation[0][1], rotation[1][0], rotation[1][1]);

            glUniformMatrix2fv(Cross::ROTATION_UNIFORM, 1, GL_FALSE, (GLfloat*)&rotation);
    ...
    public:
        ...
        void move(Direction direction) override {
            this->kinematic_ctx.horizontal.move(direction);
            this->kinematic_ctx.vertical.move(direction);
        }
        ...
        glm::vec2 get_velocity() const override {
            return this->kinematic_ctx.horizontal.get_velocity();
        }

        void rotate(Rotation direction) override {
            this->kinematic_ctx.horizontal.rotate(direction);
            this->kinematic_ctx.vertical.rotate(direction);
        }
        ...
        f32 get_angular_velocity() const override {
            return this->kinematic_ctx.horizontal.get_angular_velocity();
        }
