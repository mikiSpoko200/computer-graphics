...
void AGLWindow::GetCursorNDC(float *x, float *y) {
   ...
   *x = 2.0f*(xpos   -vtx)/vwd - 1.0f;
   *y = 2.0f*(ht-ypos-vty)/vht - 1.0f;
}

void AGLWindow::Resize( int _wd, int _ht ) {
   aspect= float(ht)/float(wd);
   auto dim = min(_ht, _wd);
   wd = dim;
   ht = dim;
...
void AGLWindow::WaitForFixedFPS(float frame_time){
   ...
   if (wait_time > 0.0) {
       std::chrono::microseconds wait_duration(int (1000000.0f * wait_time));
       std::this_thread::sleep_for(wait_duration); //before swapBuf.. and time sync 60Hz..
   }
